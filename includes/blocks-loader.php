...
class BDT_BLOCKS_LOADER {
    ...
    // generate block css
    function generate_block_css($block) {
        $style = '';
    ...
    // save to file
    function save_block_css_to_file( $css ) {
        $uploads_dir = wp_upload_dir();
        $filename = $uploads_dir['basedir'] . '/block-' . md5( $css ) . '.css';
        $url = $uploads_dir['baseurl'] . '/block-' . md5( $css ) . '.css';
    ...
    function generate_inline_style_on_render_block($block_content, $block ) {
        ...
        $filepath = $this->save_block_css_to_file( $css );
        wp_enqueue_style( $handle, $filepath, array(), time() );
